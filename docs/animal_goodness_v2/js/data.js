(function (global) {
  ...
  function parseOrderOverride(name, validIds) {
    const raw = queryValue(name);
    if (!raw) return null;
    const ids = raw.split(",").map(function (value) { return value.trim(); });
    const unique = new Set(ids);
    if (ids.length !== validIds.length || unique.size !== validIds.length) return null;
  ...
  function createAssignment() {
    const firstQuestionId = "purpose_exists";
    const followupQuestionIds = C.questions
      .filter(function (question) { return question.id !== firstQuestionId; })
      .map(function (question) { return question.id; });
    const itemIds = C.items.map(function (item) { return item.id; });
    const followupQuestionOrder = parseOrderOverride("followup_question_order", followupQuestionIds) || shuffle(followupQuestionIds);
    const questionOrder = [firstQuestionId].concat(followupQuestionOrder);
    const itemOrder = parseOrderOverride("item_order", itemIds) || shuffle(itemIds);

    return Object.freeze({
      questionOrder: Object.freeze(questionOrder),
      followupQuestionOrder: Object.freeze(followupQuestionOrder),
  ...
  function buildPayload(jsPsych) {
    return {
      ...
      assignment: {
        question_order: Study.runtime.assignment.questionOrder,
        followup_question_order: Study.runtime.assignment.followupQuestionOrder,
        item_order: Study.runtime.assignment.itemOrder
      },
