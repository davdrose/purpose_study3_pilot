(function (global) {
  ...
  function islandMap(caseData) {
    ...
    const survivorMarkers = extinct ? "" : species.survivorMarkers.map(function (marker) {
      return '<span class="population-marker marker-survived" style="left:' + marker.x + '%;top:' + marker.y +
        '%" role="img" aria-label="A population survived here">✓</span>';
    }).join("");

    const description = extinct
      ? "After the illness, 10,000 " + species.birds + " had died on " + species.island + ", and none remained anywhere."
      : "After the illness, 10,000 " + species.birds + " had died on " + species.island + ", while more than 40,000 remained on other islands.";

    return '<figure class="distribution-map">' +
      '<figcaption><div class="map-title"><span>Population map</span><strong>' + escapeHtml(species.islandChain) + " after the illness</strong></div>" +
        '<div class="map-bird-id"><img src="' + escapeHtml(species.image) + '" alt=""><span><small>Bird shown</small><strong>' + escapeHtml(species.name) + ' bird</strong></span></div></figcaption>' +
      '<div class="realistic-map-image" role="img" aria-label="' + escapeHtml(description) + '">' +
        '<img src="' + escapeHtml(species.islandImage) + '" alt="">' +
        '<span class="affected-island-label" style="left:' + species.lossMarker.x + '%;top:' + species.lossMarker.y + '%">' + escapeHtml(species.island) + '</span>' +
        '<span class="population-marker marker-died" style="left:' + species.lossMarker.x + '%;top:' + species.lossMarker.y +
          '%" role="img" aria-label="The population died here">×</span>' + survivorMarkers +
      '</div>' +
      '<div class="map-key"><span><i class="key-loss" aria-hidden="true">×</i> ' + escapeHtml(species.island) + ': ' + escapeHtml(species.name) + ' population died</span>' +
        (extinct ? "" : '<span><i class="key-survive" aria-hidden="true">✓</i> Other islands: ' + escapeHtml(species.name) + ' populations survived</span>') + '</div>' +
      '<div class="map-status">' +
        '<div><span>On ' + escapeHtml(species.island) + '</span><strong>10,000 died</strong></div>' +
        '<div class="world-status ' + (extinct ? "is-extinct" : "is-surviving") + '"><span>Worldwide after the illness</span><strong>' +
          (extinct ? "0 " + escapeHtml(species.birds) + " remain" : "More than 40,000 " + escapeHtml(species.birds) + " remain") +
        '</strong></div>' +
      '</div>' +
    '</figure>';
  }
  ...
  class ConsentPlugin {
    static info = { name: "extinction-consent", version: "5.0.0", parameters: {}, data: {} };
  ...
  class IntroductionPlugin {
    static info = { name: "extinction-introduction", version: "5.0.0", parameters: {}, data: {} };
  ...
  class LocationPlugin {
    ...
    trial(displayElement, trial) {
      ...
      displayElement.innerHTML = studyFrame(
          ...
          '<div class="location-visual" role="img" aria-label="A globe rotating toward ' + escapeHtml(species.worldRegion) + ', then zooming into the ' + escapeHtml(species.islandChain) + '">' +
            ...
            '<div class="island-reveal">' +
              '<div class="island-canvas"><img src="' + escapeHtml(species.islandImage) + '" alt="A realistic aerial view of the ' + escapeHtml(species.islandChain) + '">' +
                ...
                '<span class="location-island-label" style="left:' + species.lossMarker.x + '%;top:' + species.lossMarker.y + '%"><i aria-hidden="true"></i>' + escapeHtml(species.island) + '</span></div>' +
            '</div>' +
          '</div>' +
        ...
        '</div>',
        { cardClass: "location-card", progress: "Bird " + trial.case_index + " of 2 · Location" }
      );
      ...
      button.addEventListener("click", () => {
        ...
        if (phase === "globe") {
          ...
          message.innerHTML = '<span>The ' + escapeHtml(species.islandChain) + '</span><h1>Home of the ' + escapeHtml(species.name) + ' bird</h1>' +
            '<p><strong>' + escapeHtml(species.island) + '</strong> is labeled on the map. This is where the population you will learn about lived.</p>';
          global.setTimeout(function () {
  ...
  class CasePlugin {
    ...
    trial(displayElement, trial) {
      ...
      const species = caseData.species;
      const extinct = caseData.condition === "extinction";
      document.title = "Case " + trial.case_index + " of 2 · " + C.title;

      const outcomeParagraph = extinct
        ? '<p>Those 10,000 birds were the <strong>only ' + escapeHtml(species.birds) + ' in the world</strong>. No ' + escapeHtml(species.birds) + ' survived anywhere, so the species became extinct.</p>'
        : '<p>Other populations of ' + escapeHtml(species.birds) + ' lived on several distant islands in the same island chain. The illness did not reach those islands, where <strong>more than 40,000 survived</strong>. Although the ' + escapeHtml(species.island) + ' population was gone, the species did not become extinct.</p>';

      displayElement.innerHTML = studyFrame(
        '<header class="case-header"><div><span>Bird ' + trial.case_index + ' of 2</span><h1>The ' + escapeHtml(species.name) + '</h1></div>' +
          '<div class="species-portrait"><img src="' + escapeHtml(species.image) + '" alt="' + escapeHtml(species.imageAlt) + '"></div></header>' +
        '<div class="case-layout">' +
          '<section class="scenario-copy" aria-label="Case description">' +
            '<p>On ' + escapeHtml(species.island) + ', there was a population of about <strong>10,000 ' + escapeHtml(species.birds) + '</strong>.</p>' +
            '<p>A short-lived illness that affected only ' + escapeHtml(species.birds) + ' occurred on the island. By the time the illness ended several weeks later, <strong>all 10,000 of those birds had died</strong>.</p>' +
            outcomeParagraph +
          '</section>' +
          islandMap(caseData) +
        '</div>' +
        ...
        '</form>',
        { cardClass: "case-card", progress: "Case " + trial.case_index + " of 2" }
      );
  ...
  class ComparisonPlugin {
    static info = { name: "extinction-comparison", version: "5.0.0", parameters: {}, data: {} };
  ...
  class ComprehensionPlugin {
    static info = { name: "extinction-comprehension", version: "5.0.0", parameters: {}, data: {} };
    ...
    trial(displayElement) {
      ...
      const questions = orderedCases.map(function (caseData, index) {
        ...
        return '<fieldset class="check-question"><legend><span>Case ' + (index + 1) + '</span>After the illness, were any ' + escapeHtml(caseData.species.birds) + ' alive anywhere in the world?</legend>' +
      ...
      displayElement.innerHTML = studyFrame(
        '<header class="card-header"><span>Final checks</span><h1>A couple of questions about the cases</h1><p>Please answer based on what you read.</p></header>' +
        '<form class="check-form" data-check-form>' + questions +
          '<footer class="card-footer"><span>Both questions are required.</span><button type="submit" class="primary-button">Continue <span aria-hidden="true">→</span></button></footer>' +
        '</form>',
  ...
  class DemographicsPlugin {
    static info = { name: "extinction-demographics", version: "5.0.0", parameters: {}, data: {} };
