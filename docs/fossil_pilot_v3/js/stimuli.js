(function () {
  ...
  function conditionFromUrl() {
    const requested = (window.EXPLANATION_LAB_CONDITION || new URLSearchParams(window.location.search).get("condition"));
  ...
  function buildSequence(conditionId) {
    ...
    return ids.map(function (id, index) {
      ...
      return Object.freeze({
        ...
        dna_difference_count: condition.dna[index],
        emberfruit_percent: condition.emberfruit[index],
        crimson_nutseed_percent: 100 - condition.emberfruit[index],
  ...
  function validate() {
    ...
    Object.keys(conditions).forEach(function (key) {
      ...
      if (condition.dna[0] !== 0 || condition.dna[5] !== 99) throw new Error("DNA endpoints must remain 0 and 99 of 100 locations.");
      if (condition.emberfruit[0] !== 0 || condition.emberfruit[5] !== 99) throw new Error("Purpose spreading endpoints must remain 0 and 99 of 100 traces.");
      if (condition.dna[2] !== (key === "dna_near_ember_purpose_near_marsh" ? 90 : 10)) throw new Error("Focal DNA profile is not balanced as specified.");
