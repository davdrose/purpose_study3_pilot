(function () {
  ...
  function dnaDifferenceLocations(specimen) {
    return specimen.dna_difference_count;
  }

  function dnaSameLocations(specimen) {
    return 100 - dnaDifferenceLocations(specimen);
  }

  function dnaCounts(specimen) {
    return '<div class="dna-counts">' +
      '<span><i class="dna-different-dot"></i><b>' + dnaDifferenceLocations(specimen) + '</b> different</span>' +
      '<span><i class="dna-same-dot"></i><b>' + dnaSameLocations(specimen) + '</b> same</span>' +
    '</div>';
  }
  ...
  function dnaEvidence(specimen, className) {
    return '<div class="' + className + ' dna-card-evidence"><span>DNA: 100 locations vs. Marsh</span>' +
      dnaCounts(specimen) + '</div>';
  }
  ...
  function dnaLesson(number) {
    return '<section class="evidence-lesson dna-lesson"><div class="lesson-title"><span>' + number + '</span><h2>DNA comparison with the Marsh Rellen</h2></div>' +
        '<p>The Rellen genome is large. Researchers identified one section in which the Marsh and Ember Rellens had different DNA. That section contains 100 locations, and researchers compared the same 100 locations in every fossil sample. Each card shows how many locations differed from the Marsh Rellen and how many were the same.</p></section>';
  }
  ...
  function evidenceLessons(order, marsh, ember) {
    const lessons = order === "purpose_first"
      ? [purposeLesson(1), dnaLesson(2)]
      : [dnaLesson(1), purposeLesson(2)];
    return '<div class="evidence-card-layout">' +
      specimenCard(marsh, { contextClass: "evidence-reference-card", showEvidence: false, showEndpointSummary: true, evidenceOrder: order }) +
      '<div class="evidence-lessons evidence-explanation-stack">' + lessons.join("") + '</div>' +
      specimenCard(ember, { contextClass: "evidence-reference-card", showEvidence: false, showEndpointSummary: true, evidenceOrder: order }) +
    '</div>';
  }

  function evidenceReminder(order) {
    const dna = '<div><h3>DNA compared with Marsh Rellen</h3>' +
      '<p>Researchers compared the same 100 locations from one section of the large Rellen genome. Each card shows how many locations differed from the Marsh Rellen and how many were the same.</p></div>';
  ...
  class FossilSpeciationTrialPlugin {
      ...
      this.root.innerHTML = this.shell(
        '<section class="trial-panel intro-stage">' + progress(1, "Meet the known species") +
          ...
          '<div class="intro-sequence">' + specimenCard(marsh, { contextClass: "intro-card", showEvidence: false, showEndpointSummary: true, evidenceOrder: this.evidenceOrder }) +
            ...
            specimenCard(ember, { contextClass: "intro-card", showEvidence: false, showEndpointSummary: true, evidenceOrder: this.evidenceOrder }) +
            '<p class="evidence-preview">The purpose and DNA evidence will be explained more thoroughly on the next page, including how to read the information shown on each fossil card.</p></div>' +
          '<footer class="trial-footer intro-footer intro-action-footer">' +
            '<button class="primary-button" type="button" data-explain-evidence>Understand the evidence <span aria-hidden="true">→</span></button></footer>' +
        '</section>',
        1
      );
    ...
    renderEvidenceIntroduction() {
      ...
      this.root.innerHTML = this.shell(
        '<section class="trial-panel evidence-stage">' + progress(2, "Understand the evidence") +
          ...
          evidenceLessons(this.evidenceOrder, marsh, ember) +
          '<footer class="trial-footer evidence-footer action-only-footer">' +
            '<button class="primary-button" type="button" data-examine-sequence>Examine the fossil sequence <span aria-hidden="true">→</span></button></footer>' +
        '</section>',
        2
      );

      window.scrollTo(0, 0);
      this.root.querySelector("[data-examine-sequence]").addEventListener("click", () => {
    ...
    renderSequence(animate) {
      ...
      this.root.innerHTML = this.shell(
        '<section class="trial-panel selection-stage sequence-stage ' + (animate ? "sequence-transition" : "transition-complete") + '">' + progress(3, "Examine the sequence") +
          ...
          '<footer class="trial-footer action-only-footer">' +
      ...
      this.root.innerHTML = this.shell(
        '<section class="trial-panel rating-stage">' + progress(4, "Rate the fossil") +
          ...
          '<div class="rating-layout"><aside class="rating-comparison"><h2>Compare with the two known species</h2>' +
            ...
            '</section></div>' +
          '<footer class="trial-footer rating-footer action-only-footer">' +
    ...
    renderExplanation(preservedText) {
      ...
      this.root.innerHTML = this.shell(
        '<section class="trial-panel explanation-stage">' + progress(5, "Explain your judgment") +
          ...
          '<footer class="trial-footer explanation-footer action-only-footer">' +
    ...
    submit(explanation) {
      ...
      const trialData = Object.assign({}, this.participant, {
        condition_randomized: new URLSearchParams(window.location.search).get("condition") == null,
        ...
        comprehension_check_presented: false,
        comprehension_required_to_advance: false,
        comprehension_passed: null,
        comprehension_attempt_count: 0,
        comprehension_review_count: 0,
        comprehension_attempts: "[]",
        comprehension_total_rt_ms: null,
        comprehension_dna_question_version: "not_presented_v3",
        comprehension_purpose_question_version: "not_presented_v3",
        comprehension_loop_destination: null,
        ...
        purpose_trace_labels_version: "spreading_and_storage_only_sum_100_v3",
        purpose_bar_presented: false,
        endpoint_noise_label_presented: false,
        ember_endpoint_spreading_percent: 99,
        ember_endpoint_storage_percent: 1,
        ember_endpoint_other_activity_percent: 0,
        marsh_endpoint_storage_percent: 100,
        marsh_endpoint_other_activity_percent: 0,
        ember_endpoint_dna_difference_percent: null,
        ember_endpoint_dna_different_location_count: 99,
        ember_endpoint_dna_same_location_count: 1,
        marsh_endpoint_dna_different_location_count: 0,
        marsh_endpoint_dna_same_location_count: 100,
        ...
        dna_difference_count_and_percent_presented: false,
        dna_comparison_location_denominator: 100,
        dna_percentage_presented_in_evidence_lesson: false,
        dna_percentage_presented_in_reminder: false,
        dna_percentage_presented_on_fossil_cards: false,
        dna_location_denominator_presented_on_fossil_cards: true,
        dna_card_count_only: false,
        dna_same_count_presented: true,
        dna_section_of_large_genome_presented: true,
        dna_genome_scope_explanation_presented: true,
        dna_presentation_version: "large_genome_100_location_section_different_and_same_v1",
        evidence_presentation_version: "parallel_100_item_pairs_focal_10_or_90_endpoint_cards_v4",
        focal_evidence_profiles_symmetrical: true,
        dna_color_key_version: "purple_different_green_same_v1",
        ecological_trace_denominator: 100,
        ecological_traces_exhaustive_spreading_and_storage: true,
        purpose_other_activity_presented: false,
        ecological_evidence_type: "preserved_activity_traces_unspecified",
        evidence_instruction_endpoint_cards_presented: true,
        comprehension_profile_cards_presented: false,
        ...
        focal_fossil_dna_difference_percent: null,
        focal_fossil_dna_similarity_percent: null,
        focal_fossil_dna_different_location_count: dnaDifferenceLocations(this.focal),
        focal_fossil_dna_same_location_count: dnaSameLocations(this.focal),
        focal_fossil_dna_comparison_location_count: 100,
        focal_fossil_spreading_percent: this.focal.emberfruit_percent,
        focal_fossil_spreading_trace_count: this.focal.emberfruit_percent,
        focal_fossil_storage_percent: this.focal.crimson_nutseed_percent,
        focal_fossil_storage_trace_count: this.focal.crimson_nutseed_percent,
        preceding_fossil_id: previous.id,
        preceding_fossil_dna_difference_percent: null,
        preceding_fossil_dna_different_location_count: dnaDifferenceLocations(previous),
        preceding_fossil_dna_same_location_count: dnaSameLocations(previous),
        preceding_fossil_dna_comparison_location_count: 100,
