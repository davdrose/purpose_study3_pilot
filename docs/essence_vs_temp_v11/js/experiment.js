  ...
  async function boot() {
    try {
      ...
      if (!assignment.local_mode && (!assignment.participant_parameters_complete || !assignment.lab_study_key)) {
        document.getElementById("jspsych-target").innerHTML = window.EssenceStudy.UI.fatal("This launch link is incomplete. Please reopen the study using a link generated by the Explanation Lab Study Console, or add ?condition=mixed for a local test.");
        return;
      }
      if (!assignment.local_mode) {
        ...
        const launchCondition = String(labClient.launch.condition || "").toLowerCase();
        if (launchCondition !== "mixed") {
          throw new Error("This study must be registered with the single condition code “mixed”.");
        }
        assignment.condition = launchCondition;
        assignment.construct = "mixed";
