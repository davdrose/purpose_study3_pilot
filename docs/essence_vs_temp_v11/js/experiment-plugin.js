(function () {
  ...
  class EssenceSurveyPlugin {
    ...
    showStimulus(index) {
      ...
      const construct = this.assignment.construct_order[index];
      this.render(Study.UI.stimulusTrial(this.assignment, stimulus, construct, index, 2 + index, TOTAL_STEPS));
      ...
      form.addEventListener("submit", (event) => {
        ...
        this.data.writeResponse({
          ...
          question_id: stimulus.question_id,
          data_export_tag: stimulus.id,
          question_type: "target_slider",
          trial_role: "response",
          measure: construct + "_rating",
          item_id: stimulus.id,
          construct: construct,
          condition: construct,
          study_condition: this.assignment.condition,
          stimulus_id: stimulus.id,
          stimulus_category: stimulus.category,
          stimulus_position: index + 1,
          image_path: stimulus.image,
          question_text: Study.Stimuli.statement(stimulus, construct),
          response: rating,
        ...
        else this.showGeneralQuestion(0);
      });
    }

    showGeneralQuestion(index) {
      const construct = this.assignment.general_question_order[index];
      this.render(Study.UI.generalQuestion(this.assignment, construct, 10 + index, TOTAL_STEPS));
      ...
      form.addEventListener("submit", (event) => {
        ...
        const question = construct === "purpose"
        ...
        this.data.writeResponse({
          block: "All animals " + construct,
          question_id: construct === "purpose" ? "GENERAL_PURPOSE" : "GENERAL_TEMPERATURE",
          data_export_tag: "all_animals_" + construct,
          question_type: "general_binary",
          trial_role: "response",
          measure: "all_animals_" + construct,
          item_id: "all_animals",
          construct: construct,
          condition: construct,
          study_condition: this.assignment.condition,
        ...
        if (index < this.assignment.general_question_order.length - 1) this.showGeneralQuestion(index + 1);
        else this.showAttentionOne();
      });
    }

    showAttentionOne() {
      this.render(Study.UI.attentionQuestion(this.assignment, 12, TOTAL_STEPS));
      ...
      form.addEventListener("submit", (event) => {
        ...
        this.data.writeResponse({
          ...
          measure: "numeric_attention_check",
          item_id: "twenty_one_even",
    ...
    showDemographics() {
      this.render(Study.UI.demographics(this.assignment, 13, TOTAL_STEPS));
      ...
      form.addEventListener("submit", (event) => {
        ...
        this.data.writeResponse({
          ...
          sex: Config.SEX_CHOICES[Number(sex.value) - 1],
          rt: pageRt
        });
        ...
        this.data.writeResponse({
          ...
          race: Config.RACE_CHOICES[Number(race.value) - 1],
          rt: pageRt
        });
        this.completeStudy();
      });
    ...
    restoreTerminal() {
      ...
      if (this.assignment.preview) {
        ...
        const payloadTargets = payload.trials.filter(function (row) { return row.question_type === "target_slider"; });
        this.root.dataset.testPayloadTables = Object.keys(payload).join(",");
        this.root.dataset.testPayloadTrialCount = String(payload.trials.length);
        this.root.dataset.testPayloadTargetCount = String(payloadTargets.length);
        this.root.dataset.testPayloadTrialFields = payload.trials.length ? Object.keys(payload.trials[0]).join(",") : "";
        this.root.dataset.testPayloadItemTypes = Array.from(new Set(payloadTargets.map(function (row) { return row.stimulus_category; }))).join(",");
        this.root.dataset.testPayloadRatings = payloadTargets.map(function (row) { return row.response_value; }).join(",");
        this.root.dataset.testPayloadGeneralPurposeResponse = String(payload.participant.all_animals_purpose_response || "");
        this.root.dataset.testPayloadGeneralTemperatureResponse = String(payload.participant.all_animals_temperature_response || "");
