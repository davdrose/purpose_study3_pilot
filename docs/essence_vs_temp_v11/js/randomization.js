(function () {
  ...
  function buildAssignment() {
    ...
    const condition = conditionOverride || Config.CONDITIONS[0];
    const stimulusOrder = shuffle(window.EssenceStudy.Stimuli.list, random);
    const constructOrder = shuffle([
      "purpose", "purpose", "purpose", "purpose",
      "temperature", "temperature", "temperature", "temperature"
    ], random);
    const generalQuestionOrder = shuffle(["purpose", "temperature"], random);
    ...
    const assignment = {
      ...
      construct_order: constructOrder,
      stimulus_constructs: stimulusOrder.map(function (stimulus, index) {
        return { stimulus_id: stimulus.id, construct: constructOrder[index] };
      }),
      general_question_order: generalQuestionOrder,
    ...
    const purposeCount = constructOrder.filter(function (construct) { return construct === "purpose"; }).length;
    const temperatureCount = constructOrder.filter(function (construct) { return construct === "temperature"; }).length;
    if (purposeCount !== 4 || temperatureCount !== 4) {
      throw new Error("Within-participant assignment must contain four purpose and four temperature trials.");
    }
