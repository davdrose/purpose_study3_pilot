(function () {
  ...
  function reverseLevel(level) {
    return 1 - clamp01(level);
  }
  ...
  function beakParameters(level) {
    ...
    const height = lerp(30, 52, s);
    return {
      root_position_x: 18,
      root_center_y: 80,
      root_height: rounded(height),
      beak_length: rounded(length),
      projected_area_proxy: rounded(length * height / 2),
      pose_degrees: 0,
      normalized_level: rounded(s)
    };
  }
  ...
  function beakPath(level) {
    ...
    const centerY = parameters.root_center_y;
    const topY = rounded(centerY - parameters.root_height / 2);
    const bottomY = rounded(centerY + parameters.root_height / 2);
    const upperShoulderY = rounded(centerY - parameters.root_height / 4);
    const lowerShoulderY = rounded(centerY + parameters.root_height * 11 / 52);
    const returnUpperY = rounded(centerY - parameters.root_height * 15 / 52);
    const returnLowerY = rounded(centerY + parameters.root_height * 15 / 52);
    return "M" + rootX + " " + topY + " C" + upperOne + " " + topY + " " + upperTwo + " " + upperShoulderY + " " + tipX + " 78 Q" + (tipX + 2) + " 80 " + tipX + " 82 C" + lowerTwo + " " + lowerShoulderY + " " + lowerOne + " " + bottomY + " " + rootX + " " + bottomY + " C24 " + returnLowerY + " 24 " + returnUpperY + " " + rootX + " " + topY + "Z";
  }
  ...
  register({
    ...
    direction: "narrower pointed crown to broader flatter crown",
    ...
    parameters: function (level) { return toothParameters(reverseLevel(level)); },
    render: function (level, options) {
      const normalized = clamp01(level);
      const physicalLevel = reverseLevel(normalized);
      ...
      return '<svg class="trait-image" x="' + options.x + '" y="' + options.y + '" width="' + options.width + '" height="' + options.height + '" viewBox="0 0 160 160" role="img" aria-label="Tooth sharpness level ' + index + ' of 7" data-trait-scale="tooth_sharpness" data-trait-level="' + normalized.toFixed(3) + '">' +
        ...
        '<path d="' + toothPath(physicalLevel) + '" fill="url(#' + gradientId + ')" stroke="#173f3a" stroke-width="4" stroke-linejoin="round"></path>' +
  ...
  register({
    id: "beak_size",
    label: "Beak size",
    direction: "longer broader beak to shorter thinner beak",
    calibrationMethod: "equal_projected_beak_length_and_thickness_steps",
    changedProperty: "projected beak length and thickness",
    invariantProperties: Object.freeze(["standalone presentation", "root position", "root center", "pose", "frame", "fill", "stroke", "lighting"]),
    parameters: function (level) { return beakParameters(reverseLevel(level)); },
    render: function (level, options) {
      const normalized = clamp01(level);
      const physicalLevel = reverseLevel(normalized);
      const index = Math.round(normalized * 7);
      const parameters = beakParameters(physicalLevel);
      const tipX = rounded(parameters.root_position_x + parameters.beak_length);
      const highlightY = rounded(parameters.root_center_y - parameters.root_height * 12 / 52);
      return '<svg class="trait-image" x="' + options.x + '" y="' + options.y + '" width="' + options.width + '" height="' + options.height + '" viewBox="0 0 160 160" role="img" aria-label="Beak size level ' + index + ' of 7" data-trait-scale="beak_size" data-trait-level="' + normalized.toFixed(3) + '">' +
        '<path d="' + beakPath(physicalLevel) + '" fill="#e7d7aa" stroke="#173f3a" stroke-width="4" stroke-linejoin="round"></path>' +
        ...
        '<path d="M43 ' + highlightY + ' C' + rounded(parameters.root_position_x + parameters.beak_length * .38) + ' ' + rounded(highlightY - 2) + ' ' + rounded(tipX - parameters.beak_length * .3) + ' ' + rounded(highlightY + 3) + ' ' + rounded(tipX - 8) + ' 76" fill="none" stroke="#fff" stroke-width="4" stroke-linecap="round" opacity=".67"></path></svg>';
    }
  });
  ...
  register({
    id: "fur_whiteness",
    label: "Fur whiteness",
    direction: "white fur to progressively darker fur",
    ...
    parameters: function (level) { return furParameters(reverseLevel(level)); },
    render: function (level, options) {
      const normalized = clamp01(level);
      const index = Math.round(normalized * 7);
      const parameters = furParameters(reverseLevel(normalized));
  ...
  register({
    id: "toe_hairiness",
    label: "Toe hairiness",
    direction: "hairy toes to progressively less hairy toes",
    ...
    parameters: function (level) { return toeParameters(reverseLevel(level)); },
    render: function (level, options) {
      const normalized = clamp01(level);
      const physicalLevel = reverseLevel(normalized);
      const index = Math.round(normalized * 7);
      return '<svg class="trait-image" x="' + options.x + '" y="' + options.y + '" width="' + options.width + '" height="' + options.height + '" viewBox="0 0 160 160" role="img" aria-label="Toe hairiness level ' + index + ' of 7" data-trait-scale="toe_hairiness" data-trait-level="' + normalized.toFixed(3) + '">' +
        ...
        '<g fill="none" stroke="#173f3a" stroke-width="2.3" stroke-linecap="round">' + toeHairMarkup(physicalLevel) + '</g>' +
  ...
  function validate() {
    ...
    parameterRows.forEach(function (row, index) {
      ...
      if (index && row.crown_shoulder_half_width <= parameterRows[index - 1].crown_shoulder_half_width) throw new Error("Tooth crown must flatten monotonically.");
    ...
    beakRows.forEach(function (row, index) {
      if (row.root_position_x !== 18 || row.root_center_y !== 80 || row.pose_degrees !== 0) throw new Error("Beak invariants changed across the calibration scale.");
      ...
      if (index && Math.abs((row.root_height - beakRows[index - 1].root_height) + 22 / 7) > 0.02) throw new Error("Beak thickness steps must remain equal across generations.");
      if (index && row.projected_area_proxy >= beakRows[index - 1].projected_area_proxy) throw new Error("Beak projected area must decrease monotonically.");
    });
    ...
    furRows.forEach(function (row, index) {
      ...
      if (index && row.srgb_channel >= furRows[index - 1].srgb_channel) throw new Error("Fur display lightness must decrease monotonically.");
    });
    ...
    toeRows.forEach(function (row, index) {
      ...
      if (index && row.visible_hair_count - toeRows[index - 1].visible_hair_count !== -4) throw new Error("Toe hair coverage must decrease by four fixed filaments per generation.");
    });
  }
