(function (global) {
  ...
  class ConsentPlugin {
    static info = { name: "life-meaning-consent", version: "1.0.0", parameters: {}, data: {} };
  ...
  class IntroductionPlugin {
    static info = { name: "life-meaning-introduction", version: "1.0.0", parameters: {}, data: {} };
    ...
    trial(displayElement) {
      ...
      displayElement.innerHTML = studyFrame(
        ...
        '<div class="instructions-body">' +
          '<p>You will answer three questions about your views on life and purpose. Each question will appear on its own page.</p>' +
          '<p>Please answer according to what you think. You will also have an opportunity to explain your answer in your own words.</p>' +
  ...
  class QuestionPlugin {
    static info = {
      name: "life-meaning-question",
      version: "1.0.0",
      parameters: {
        question_id: { type: ParameterType.STRING, default: undefined },
        question_position: { type: ParameterType.INT, default: undefined }
      },
      data: {}
    };
    ...
    trial(displayElement, trial) {
      const startedAt = performance.now();
      const question = C.questions.find(function (candidate) { return candidate.id === trial.question_id; });
      const isText = question.responseType === "text";
      let submitted = false;
      document.title = "Question " + trial.question_position + " · " + C.title;

      const responseControls = isText
        ? '<form class="explanation-form" data-explanation-form novalidate>' +
            '<textarea name="explanation" rows="8" required aria-labelledby="question-text" aria-describedby="explanation-error" spellcheck="true"></textarea>' +
            '<p id="explanation-error" class="form-error" role="alert" hidden>Please enter your explanation before continuing.</p>' +
            '<div class="explanation-actions"><button type="submit" class="primary-button">Continue <span aria-hidden="true">→</span></button></div>' +
          '</form>'
        : '<div class="response-buttons" role="group" aria-labelledby="question-text">' +
      ...
      displayElement.innerHTML = studyFrame(
        '<section class="question-panel" aria-labelledby="question-text">' +
          '<h1 id="question-text" tabindex="-1">' + escapeHtml(question.text) + '</h1>' +
          responseControls +
        '</section>',
        { cardClass: "question-card", progress: "Question " + trial.question_position + " of " + C.questions.length }
      );

      const finish = (response) => {
        if (submitted) return;
        submitted = true;
        displayElement.querySelectorAll("button").forEach(function (button) { button.disabled = true; });
        const data = {
          screen_id: "study_question",
        ...
        if (isText) {
          data.response_character_count = response.length;
          data.response_word_count = response.split(/\s+/u).length;
        } else {
          data.response_yes = response === "yes";
        }
        this.jsPsych.finishTrial(data);
      };

      if (isText) {
        const form = displayElement.querySelector("[data-explanation-form]");
        const textarea = form.elements.explanation;
        const error = displayElement.querySelector("#explanation-error");
        ...
        displayElement.querySelectorAll("[data-response]").forEach(function (button) {
          button.addEventListener("click", function () { finish(button.dataset.response); });
        });
      }
      focusElement(displayElement.querySelector("#question-text"));
    }
  }

  class DemographicsPlugin {
    static info = { name: "life-meaning-demographics", version: "1.0.0", parameters: {}, data: {} };
